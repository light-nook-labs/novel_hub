...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if novel_count == 0:
            self.stdout.write(
                self.style.WARNING("No novels in database. Run init_db first.")
            )
            return

        if tag_count == 0:
            self.stdout.write(
                self.style.WARNING("No tags in database. Run init_db first.")
            )
            return
        ...
        if m2m_count > 0 and check_only:
            # Show some stats
            novels_with_tags = (
                Novel.tags.through.objects.values("novel_id").distinct().count()
            )
        ...
        logger.info(
            "Built %d M2M pairs (%d novels not in DB, %d tags not in DB)",
            len(novel_tags),
            skipped_novels,
            len(skipped_tags),
        )
        ...
        with transaction.atomic():
            if is_postgres:
                ...
                with connection.cursor() as cursor:
                    inserted = 0
                    for i in progress(
                        range(0, len(novel_tags), batch_size), desc="Inserting"
                    ):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Fixed! M2M relationships: {m2m_count} → {final_count}")
        )
